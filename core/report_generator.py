    ...
    async def generate_report(
        ...
        try:
            ...
            generator_logger.info(
                f"Начало генерации отчета для {geo}, период {days_back} дней")
            ...
            generator_logger.info(f"DEBUG: После парсинга, перед классификацией. Новостей: {len(news_items)}")
            ...
            generator_logger.info(f"DEBUG: После yield, перед вызовом AI")
            generator_logger.info(f"Вызов AI агента для классификации {len(news_items)} новостей")
            try:
                inforeasons = await self.ai_agent.classify_inforeasons(
                    news_items=news_items,
                    geo=geo
                )
                generator_logger.info(f"AI агент вернул {len(inforeasons)} инфоповодов")
            except Exception as e:
                generator_logger.exception(f"Ошибка при вызове AI агента: {e}")
                yield {
                    "stage": "classification",
                    "status": "error",
                    "message": f"❌ Ошибка AI: {str(e)[:100]}",
                    "data": {"error": str(e)}
                }
                return
            ...
            try:
                ...
                generator_logger.exception(f"Ошибка записи в Google Sheets: {e}")
                sheet_url = None
                raise  # Пробрасываем ошибку выше, чтобы пользователь знал о проблеме
