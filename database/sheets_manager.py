...
class SheetsManager:
    ...
    def write_block_1_inforeasons(
        ...
        # Заголовки колонок
        headers = ["Заголовок", "Источник", "Дата", "Категория", "Описание", "Триггер", "Срочность"]
    ...
    def write_block_2_angles(
        ...
        # Заголовки колонок
        headers = ["ID", "Инфоповод", "Угол", "Связь с офером", "Боль", "Тип", "Приоритет"]
        for col, header in enumerate(headers, 1):
            worksheet.update_cell(start_row + 1, col, header)
    ...
    def write_block_4_recommendations(
        ...
        # Заголовок блока
        worksheet.update_cell(start_row, 1, "🎯 БЛОК 4: ТОП-5 РЕКОМЕНДАЦИЙ К ТЕСТУ")
    ...
    def write_block_5_risks(
        ...
        # Заголовки колонок
        headers = ["Инфоповод #", "Юридический", "Бан платформой", "Негатив аудитории", "Срок"]
        for col, header in enumerate(headers, 1):
            worksheet.update_cell(start_row + 1, col, header)
    ...
    def _format_block_header(self, worksheet, row: int):
        ...
        try:
            ...
            sheets_logger.warning(f"Ошибка форматирования заголовка блока: {e}")
    ...
    def write_report(self, report: Report) -> str:
        ...
        current_row = self.write_block_1_inforeasons(worksheet, report.inforeasons, current_row)
        current_row = self.write_block_2_angles(worksheet, report.angles, current_row)
        current_row = self.write_block_3_headlines(worksheet, report.headlines, current_row)
        
        if report.recommendations:
            current_row = self.write_block_4_recommendations(worksheet, report.recommendations, current_row)
        
        if report.risks:
            current_row = self.write_block_5_risks(worksheet, report.risks, current_row)
        
        if report.urgency_board:
            current_row = self.write_block_6_urgency(worksheet, report.urgency_board, current_row)
